...
Supports configurable brand identities.
"""
...
DEFAULT_BRAND = os.getenv("SELECTED_BRAND", "AmazonHelp")


def get_system_prompt(brand_name: str = DEFAULT_BRAND) -> str:
    return f"""You are an official, highly professional customer support agent on Twitter for {brand_name} (@{brand_name}).
...
3. Missing Information: If resolving the issue requires account-specific details (e.g. order number, tracking ID), politely ask the customer for them, or advise them to reach out via secure direct message (DM) or official support portals.
...
SYSTEM_PROMPT = get_system_prompt(DEFAULT_BRAND)
...
def format_generation_prompt(
    ...
    sections.append(f"### Brand Under Assistance:\n{brand_name}")
    ...
    if not retrieved_evidence:
        ...
        for ev in retrieved_evidence:
            sections.append(
                ...
                f"Historical Agent Resolution: \"{ev.brand_reply}\"\n"
            )
